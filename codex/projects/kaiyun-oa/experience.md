...
## 2026-07-29 文件持久化状态事务边界

- 场景：业务保存或提交审批会批量持久化临时文件，后续业务步骤失败时不能遗留永久的 `PROCESSING`。
- 做法：持久化抢占、持久化元数据和成功或失败状态参加同一个业务事务；外层业务或批量后续文件失败时整体回滚，使临时文件可以重试。
- 注意：持久化抢占不能使用 `REQUIRES_NEW` 独立提交，否则外层回滚只会撤销元数据和完成状态，却保留 `PROCESSING`；过期文件清理的 `CLEANING` 独立抢占仍按清理任务的显式释放机制处理。
...
## 2026-08-30 成本汇总空指针排查与修复

- 场景：生产环境 `/cost/summary` 仅记录无堆栈的空指针，公司成本因残留部门 ID 被部门汇总误查后触发 `type=null` 比较异常。
- 做法：联合请求日志、部署版本源码和只读 SQL 缩小异常数据范围；部门汇总按 `costCategory=DEPARTMENT` 隔离公司成本，有限值比较使用空安全方式，并用部门、个人汇总测试覆盖空类型记录。
- 注意：公司成本与部门成本都可能使用 `isUser=NO`，不能只依赖该字段区分；项目 Surefire 2.17 不执行 JUnit 5，回归测试使用 JUnit 4，并确认测试数量不为零；本地内部模块落后时使用 `-am` 沿依赖链验证。
